...
Rumina 支持使用 Cranelift 进行 JIT (Just-In-Time) 编译来提升性能。JIT 功能通过 feature flag 控制：

```bash
# 使用 JIT 功能构建（仅支持非 WASM 平台）
...
# 运行性能基准测试
cargo test --release --features jit jit_performance
```

**JIT 优化特性：**
- 热路径检测：自动识别经常执行的代码路径（循环、函数等）
- 动态编译：将热路径编译为本机代码以提高执行速度
- 内联缓存：优化对象成员访问性能
- 指令融合：合并相邻的简单操作以减少指令数
- 常量折叠：在编译时计算常量表达式
...
**注意事项：**
- JIT 编译需要一定的预热时间（约 100 次执行后触发）
- 仅在非 WASM 平台可用（Linux、macOS、Windows）
- 默认情况下 JIT 功能是禁用的，需要显式启用
...
- **预分配栈空间**：减少动态内存分配
- **FxHashMap**：使用更快的哈希表实现
- **内联缓存**：加速对象成员访问
- **指令索引访问**：避免克隆操作码
- **递归深度限制**：防止栈溢出（默认 4000 层）
